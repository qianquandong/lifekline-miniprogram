...
function generatePrompt(baziResult) {
  ...
  const prompt = `请作为一位专业的八字命理师，对以下八字进行深度分析。
...
### 3. 财富分析（wealth）
分析财运特点、财富积累方式、理财建议、财富运势等，字数控制在200-300字。

### 4. 婚姻分析（marriage）
分析感情特点、婚姻运势、配偶特征、感情建议等，字数控制在200-300字。
...
注意：
1. fortune数组必须包含1-100岁共100个数据，不能遗漏
2. score的取值范围是0-100
3. trend只能是"up"、"down"或"flat"之一
4. 请确保JSON格式完全正确，可以直接被解析
5. 运势曲线应该有一定的逻辑性，考虑大运周期的影响`;
...
function generatePromptPreview(baziResult) {
  ...
  return `八字：${year.full} ${month.full} ${day.full} ${hour.full}
性别：${genderText}
